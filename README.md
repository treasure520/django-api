...
First rename or link the file **`env.example`** to **`.env`** and configure variables. Replace the values you need:
```
# MySQL
...
```
### Update user
* request with your API token in HTTP Header. Suppose we update the fields job_title and mobile.
```bash
